...
export async function generateHomeProperties(json) {
  try {
    const imoveis = json.slice(0, 15).map((item) => {
    ...
    fs.writeFileSync(
      "./public/data/properties-home.json",
    ...
    console.log(`   └── Tamanho aproximado: ${(fs.statSync("./public/data/properties-home.json").size / 1024 / 1024).toFixed(10)} MB`,);
    console.log('');
  } catch (error) {
